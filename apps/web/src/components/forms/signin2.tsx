...
// Assuming you're using this for notifications
...
export const LoginForm = () => {
  ...
  return (
    <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(onFormSubmit)}>
      <div>
        <label htmlFor="email" className="mb-2 block text-sm font-light text-gray-900">
          Your email
        </label>
        <input
          {...register('email')}
          type="email"
          id="email"
          className="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-black placeholder:text-gray-500 focus:border-black focus:ring-black sm:text-sm"
      ...
      <div>
        <label htmlFor="password" className="mb-2 block text-sm font-light text-gray-900">
          Password
        </label>
        <input
          {...register('password')}
